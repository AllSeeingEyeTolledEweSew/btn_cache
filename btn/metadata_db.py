...
def _te_json_to_rows(entry: api_types.TorrentEntry) -> _Rows:
    ...
    annotations = api_types.TorrentEntry.__annotations__  \
...
def _migrate_1(conn:apsw.Connection, schema:str) -> None:
    sql = importlib.resources.read_text("btn.sql", "metadata_1.0.0.sql")
...
def _update_series(conn:apsw.Connection, *rows: _SeriesRow) -> None:
    ...
    cols = rows[0].keys()
    query = "".join(
        ("insert into series (", ", ".join(cols), ") values (",
         ", ".join(f":{k}"
                   for k in cols), ") on conflict (id) do update set ",
         ", ".join(f"{k} = :{k}" for k in cols
                   if k != "id"), " where ", " or ".join(f"{k} is not :{k}"
                                                         for k in cols
                                                         if k != "id")))
...
def _update_groups(conn:apsw.Connection, *rows: _GroupRow) -> None:
    ...
    cols = rows[0].keys()
    query = "".join(
        ("insert into torrent_entry_group (", ", ".join(cols),
...
def _update_torrent_entries(conn:apsw.Connection, *rows: _TorrentEntryRow) -> None:
    if not rows:
        return
    cur = conn.cursor()
    cols = rows[0].keys()
    query = "".join(
        ("insert into torrent_entry (", ", ".join(cols), ") values (",
         ", ".join(f":{k}"
                   for k in cols), ") on conflict (id) do update set ",
         ", ".join(f"{k} = :{k}" for k in cols
                   if k != "id"), " where ", " or ".join(f"{k} is not :{k}"
...
class UnfilteredGetTorrentsResultUpdate(TorrentEntriesUpdate):
    ...
    def __init__(self, offset:int, result: api_types.GetTorrentsResult) -> None:
        ...
        self._ordered_te_rows = sorted(self._te_rows, 
    ...
    def apply(self, conn:apsw.Connection) -> None:
        ...
        try:
            cur.executemany("insert into temp.ids (id) values (?)",
...
class TorrentInfoUpdate:
    ...
    def __init__(self, torrent_entry_id: int,
            info: Union[bytes, memoryview, bytearray]) -> None:
        # TODO: optimize
        ...
        if b"files" in info_dict:
            ...
            utf8 = b"name.utf-8" in info_dict and all(b"path.utf-8" in v
                                                      for v in files)
            for index, file_dict in enumerate(files):
                ...
                path = [name] + path
                self._rows.append(
                    _FileInfoRow(id=torrent_entry_id,
                                 file_index=index,
    ...
    def apply(self, conn:apsw.Connection) -> None:
        ...
        query = "".join(("insert into file_info (", ",".join(cols),
                         ") values (", ",".join(f":{k}" for k in cols),
...
class TorrentFileUpdate:
    ...
    def __init__(self, torrent_entry_id: int,
            torrent_file: Union[bytes, memoryview, bytearray]) -> None:
        # TODO: optimize
