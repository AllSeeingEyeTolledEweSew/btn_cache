...
class UserAccess:
    def __init__(self,
        ...
        for prefix in ("https://broadcasthe.net", "http://broadcasthe.net"):
            ratelimit.ratelimit_session(self._session, prefix,
                                        self._rate_limiter)
        self._auth = auth
    ...
    def get_feed(self, name: str) -> requests.Response:
        if (self._auth.auth is None or self._auth.user_id is None
                or self._auth.authkey is None or self._auth.passkey is None):
            raise ValueError()
