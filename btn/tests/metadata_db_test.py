...
class TableState:
    def __init__(self, conn: apsw.Connection, table: str) -> None:
        ...
        self.updated_at = dict(
            cast(Iterable[Tuple[int, int]],
                 cur.execute(f"select id, updated_at from {table}")))
        self.deleted = set(i for i, in cast(
            Iterable[Tuple[int]],
...
class TableStateDelta:
    ...
    def diff(cls: Type[_D], a: TableState, b: TableState) -> _D:
        ...
        went_backward = {
            i
            for i in a.updated_at if a.updated_at[i] > b.updated_at[i]}
        ...
        return cls(disappeared=disappeared,
...
class ChangeChecker:
    ...
    def series(
        ...
        *,
        ...
        self.expected_series_delta.set(modified=ids,
                                       new=new,
                                       deleted=delete,
                                       undeleted=undelete)
        return self
    ...
    def group(
        ...
        *,
        ...
        self.expected_group_delta.set(modified=ids,
                                      new=new,
                                      deleted=delete,
                                      undeleted=undelete)
        return self
    ...
    def entry(
        ...
        *,
        ...
        self.expected_entry_delta.set(modified=ids,
                                      new=new,
                                      deleted=delete,
                                      undeleted=undelete)
        return self

    def check(self, table: str, a_state: TableState,
              expected: TableStateDelta) -> None:
    ...
    def __exit__(self, exc_type: Any, exc_value: Any, exc_tb: Any) -> None:
        ...
        self.check("torrent_entry_group", self.a_group,
                   self.expected_group_delta)
...
class BaseMetadataTest(lib.BaseTest):
    ...
    def update_search_result(self, offset:int, result:api_types.GetTorrentsResult) -> None:
...
class UpdateTorrentEntryTest(BaseMetadataTest):
    ...
    def test_new_series(self) -> None:
        ...
        with self.assert_changes() as change:
            change.series(new=(self.series_id + 1, ),
                          delete=(self.series_id, ))
    ...
    def test_empty_values(self) -> None:
        ...
        self.update_entry(self.entry)
        values = cur.execute(
            "select imdb_id from series").fetchall()
        self.assertEqual(values, [(None, )])

        self.entry["Series"] = ""
        self.update_entry(self.entry)
        values = cur.execute(
            "select name from series").fetchall()
        self.assertEqual(values, [(None, )])

        self.entry["SeriesBanner"] = ""
        self.update_entry(self.entry)
        values = cur.execute(
            "select banner from series").fetchall()
        self.assertEqual(values, [(None, )])

        self.entry["SeriesPoster"] = ""
        self.update_entry(self.entry)
        values = cur.execute(
            "select poster from series").fetchall()
        self.assertEqual(values, [(None, )])

        self.entry["TvdbID"] = ""
        self.update_entry(self.entry)
        values = cur.execute(
            "select tvdb_id from series").fetchall()
        self.assertEqual(values, [(None, )])

        self.entry["TvrageID"] = ""
        self.update_entry(self.entry)
        values = cur.execute(
            "select tvrage_id from series").fetchall()
        self.assertEqual(values, [(None, )])

        self.entry["YoutubeTrailer"] = ""
        self.update_entry(self.entry)
        values = cur.execute(
            "select youtube_trailer from series").fetchall()
        self.assertEqual(values, [(None, )])

        self.entry["GroupName"] = ""
        self.update_entry(self.entry)
        values = cur.execute(
            "select name from torrent_entry_group").fetchall()
        self.assertEqual(values, [(None, )])

        self.entry["Codec"] = ""
        self.update_entry(self.entry)
        values = cur.execute(
            "select codec from torrent_entry").fetchall()
        self.assertEqual(values, [(None, )])

        self.entry["Container"] = ""
        self.update_entry(self.entry)
        values = cur.execute(
            "select container from torrent_entry").fetchall()
        self.assertEqual(values, [(None, )])

        self.entry["Origin"] = ""
        self.update_entry(self.entry)
        values = cur.execute(
            "select origin from torrent_entry").fetchall()
        self.assertEqual(values, [(None, )])

        self.entry["ReleaseName"] = ""
        self.update_entry(self.entry)
        values = cur.execute(
            "select release_name from torrent_entry").fetchall()
        self.assertEqual(values, [(None, )])

        self.entry["Resolution"] = ""
        self.update_entry(self.entry)
        values = cur.execute(
            "select resolution from torrent_entry").fetchall()
        self.assertEqual(values, [(None, )])

        self.entry["Source"] = ""
        self.update_entry(self.entry)
        values = cur.execute(
            "select source from torrent_entry").fetchall()
        self.assertEqual(values, [(None, )])
...
class UpdateTorrentInfoTest(BaseMetadataTest):
    def assert_file_info(
            self, expected: List[Tuple[int, int, bytes, Optional[str], int,
                                       int]]) -> None:
        got = self.conn.cursor().execute(
            "select * from file_info").fetchall()
        self.assertEqual(got, expected)

    def check(self, info: Dict[bytes, Any],
              expected: List[Tuple[int, int, bytes, Optional[str], int, int]]) -> None:
        self.update_entry(self.entry)
        with self.assert_changes() as change:
            change.entry(ids=(self.entry_id, ))
            self.update_torrent_info(self.entry_id,
                                        better_bencode.dumps(info))
        self.assert_file_info(expected)

    def test_single_nonutf8(self) -> None:
        self.check({
            b"name": b"test.txt",
            b"length": 123}, [(self.entry_id, 0, b"l8:test.txte", None, 0, 123)])
        ...
        self.check(
            {
                ...
                b"files": [{
                    ...
                    b"length": 100}, {
                        ...
                        b"length": 50}]},
            [(self.entry_id, 0, b"l10:test.utf-87:a.utf-88:test.txte", "utf-8", 0,
              100),
             (self.entry_id, 1, b"l10:test.utf-87:b.utf-89:other.txte", "utf-8", 100,
              150)])
